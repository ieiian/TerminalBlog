...
<body>
    ...
    <div class="term-footer">
        <p>© 2026 TerminalBlog · 终端黑客风格 · Powered by Cloudflare Pages + KV</p>
    ...
    async function renderHome(body) {
        ...
        const [stats, posts] = await Promise.all([
            apiGet('/stats'),
            apiGet('/posts?page=1&limit=' + POSTS_PER_PAGE)
        ]);
...
|_| \\\\_\\\\\\\\____|___|_| \\\\_\\\\_____|___|_| \\\\_|_| \\\\_|_____|_| \\\\_\\\\
    ...
    async function goPage(page) {
        currentPage = page;
        const body = document.getElementById('termBody');
        // Re-render just the post list portion
        const posts = await apiGet(\`/posts?page=\${page}&limit=\${POSTS_PER_PAGE}\`);
        // Full re-render for simplicity
        if (currentView === 'home') renderHome(body);
        else if (currentView === 'tag') renderTagPosts(body);
    }
 ...
 ╚╩╝└─┘└─┘╚═╝╚═╝    ╩ ╩┴ ┴┘└┘└─┘└─┘┴─┘└─┘
        ...
        try {
            ...
            const listEl = document.getElementById('adminPostList');
            if (listEl) listEl.innerHTML = \`<p class="error-text">加载失败: \${escapeHtml(e.message)}</p>\`;
        }
    }
